...
public class Volatility_ATR_1 implements TechnicalIndicator {

    @Override
    public String getName() { return "Volatility ATR_1"; }

    @Override
    public String getCategory() { return "volatility"; }

    @Override
    public String getDescription() { return "Volatility ATR_1 - volatility indicator with period 1"; }
    ...
    public IndicatorResult calculate(List<Candle> candles, String symbol, String timeframe) {
        ...
        double value = IndicatorMath.stddev(closes, Math.min(1, closes.size()));
        double price = candles.get(candles.size()-1).getClose();
        double mean = IndicatorMath.sma(closes, Math.min(1, closes.size()));
        ...
        return new IndicatorResult(getName(), symbol, timeframe,
            ...
            "Volatility ATR_1: value=" + String.format("%.5f", value) + " signal=" + signal);
    }
}
