...
public class MarketProfile_Fair_Value_Gap_Down implements TechnicalIndicator {

    @Override
    public String getName() { return "Market Profile Fair_Value_Gap_Down"; }

    @Override
    public String getCategory() { return "market_profile"; }

    @Override
    public String getDescription() { return "Market Profile Fair_Value_Gap_Down - market_profile indicator with period 3"; }

    @Override
    public int getMinCandles() { return 3; }
    ...
    public IndicatorResult calculate(List<Candle> candles, String symbol, String timeframe) {
        ...
        double value = IndicatorMath.sma(closes, Math.min(3, closes.size()));
        double high = IndicatorMath.max(closes, Math.min(3, closes.size()));
        double low = IndicatorMath.min(closes, Math.min(3, closes.size()));
        ...
        return new IndicatorResult(getName(), symbol, timeframe,
            ...
            "Market Profile Fair_Value_Gap_Down: value=" + String.format("%.5f", value) + " signal=" + signal);
    }
}
