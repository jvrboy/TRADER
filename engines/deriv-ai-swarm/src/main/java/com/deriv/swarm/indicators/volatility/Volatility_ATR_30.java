...
public class Volatility_ATR_30 implements TechnicalIndicator {

    @Override
    public String getName() { return "Volatility ATR_30"; }

    @Override
    public String getCategory() { return "volatility"; }

    @Override
    public String getDescription() { return "Volatility ATR_30 - volatility indicator with period 30"; }
    ...
    public IndicatorResult calculate(List<Candle> candles, String symbol, String timeframe) {
        ...
        double value = IndicatorMath.stddev(closes, Math.min(30, closes.size()));
        double price = candles.get(candles.size()-1).getClose();
        double mean = IndicatorMath.sma(closes, Math.min(30, closes.size()));
        ...
        return new IndicatorResult(getName(), symbol, timeframe,
            ...
            "Volatility ATR_30: value=" + String.format("%.5f", value) + " signal=" + signal);
    }
}
