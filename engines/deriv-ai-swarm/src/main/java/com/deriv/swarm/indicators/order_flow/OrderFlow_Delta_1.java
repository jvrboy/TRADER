...
public class OrderFlow_Delta_1 implements TechnicalIndicator {

    @Override
    public String getName() { return "Order Flow Delta_1"; }

    @Override
    public String getCategory() { return "order_flow"; }

    @Override
    public String getDescription() { return "Order Flow Delta_1 - order_flow indicator with period 1"; }
    ...
    public IndicatorResult calculate(List<Candle> candles, String symbol, String timeframe) {
        ...
        int len = Math.min(1, closes.size());
        double delta = 0;
        for (int i = 0; i < len; i++) {
        ...
        return new IndicatorResult(getName(), symbol, timeframe,
            ...
            "Order Flow Delta_1: value=" + String.format("%.5f", value) + " signal=" + signal);
    }
}
