...
public class MarketProfile_Opening_Range_Breakout_Down implements TechnicalIndicator {

    @Override
    public String getName() { return "Market Profile Opening_Range_Breakout_Down"; }

    @Override
    public String getCategory() { return "market_profile"; }

    @Override
    public String getDescription() { return "Market Profile Opening_Range_Breakout_Down - market_profile indicator with period 2"; }
    ...
    public IndicatorResult calculate(List<Candle> candles, String symbol, String timeframe) {
        ...
        double value = IndicatorMath.sma(closes, Math.min(2, closes.size()));
        double high = IndicatorMath.max(closes, Math.min(2, closes.size()));
        double low = IndicatorMath.min(closes, Math.min(2, closes.size()));
        ...
        return new IndicatorResult(getName(), symbol, timeframe,
            ...
            "Market Profile Opening_Range_Breakout_Down: value=" + String.format("%.5f", value) + " signal=" + signal);
    }
}
