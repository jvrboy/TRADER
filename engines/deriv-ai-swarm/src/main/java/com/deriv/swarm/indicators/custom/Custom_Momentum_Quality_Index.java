...
public class Custom_Momentum_Quality_Index implements TechnicalIndicator {

    @Override
    public String getName() { return "Custom Momentum_Quality_Index"; }

    @Override
    public String getCategory() { return "custom"; }

    @Override
    public String getDescription() { return "Custom Momentum_Quality_Index - custom indicator with period 14"; }
    ...
    public IndicatorResult calculate(List<Candle> candles, String symbol, String timeframe) {
        ...
        double sma = IndicatorMath.sma(closes, Math.min(14, closes.size()));
        double rsi = IndicatorMath.rsi(closes, Math.min(14, closes.size()));
        double std = IndicatorMath.stddev(closes, Math.min(14, closes.size()));
        ...
        return new IndicatorResult(getName(), symbol, timeframe,
            ...
            "Custom Momentum_Quality_Index: value=" + String.format("%.5f", value) + " signal=" + signal);
    }
}
