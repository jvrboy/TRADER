...
public class Custom_Multi_Timeframe_MACD implements TechnicalIndicator {

    @Override
    public String getName() { return "Custom Multi_Timeframe_MACD"; }

    @Override
    public String getCategory() { return "custom"; }

    @Override
    public String getDescription() { return "Custom Multi_Timeframe_MACD - custom indicator with period 26"; }
    ...
    public IndicatorResult calculate(List<Candle> candles, String symbol, String timeframe) {
        ...
        double sma = IndicatorMath.sma(closes, Math.min(26, closes.size()));
        double rsi = IndicatorMath.rsi(closes, Math.min(26, closes.size()));
        double std = IndicatorMath.stddev(closes, Math.min(26, closes.size()));
        ...
        return new IndicatorResult(getName(), symbol, timeframe,
            ...
            "Custom Multi_Timeframe_MACD: value=" + String.format("%.5f", value) + " signal=" + signal);
    }
}
