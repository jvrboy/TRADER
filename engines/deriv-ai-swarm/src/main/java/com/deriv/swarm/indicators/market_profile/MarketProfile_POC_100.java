...
public class MarketProfile_POC_100 implements TechnicalIndicator {

    @Override
    public String getName() { return "Market Profile POC_100"; }

    @Override
    public String getCategory() { return "market_profile"; }

    @Override
    public String getDescription() { return "Market Profile POC_100 - market_profile indicator with period 100"; }

    @Override
    public int getMinCandles() { return 100; }
    ...
    public IndicatorResult calculate(List<Candle> candles, String symbol, String timeframe) {
        ...
        double value = IndicatorMath.sma(closes, Math.min(100, closes.size()));
        double high = IndicatorMath.max(closes, Math.min(100, closes.size()));
        double low = IndicatorMath.min(closes, Math.min(100, closes.size()));
        ...
        return new IndicatorResult(getName(), symbol, timeframe,
            ...
            "Market Profile POC_100: value=" + String.format("%.5f", value) + " signal=" + signal);
    }
}
