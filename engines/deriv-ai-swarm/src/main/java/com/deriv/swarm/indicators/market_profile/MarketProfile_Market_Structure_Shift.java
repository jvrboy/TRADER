...
public class MarketProfile_Market_Structure_Shift implements TechnicalIndicator {

    @Override
    public String getName() { return "Market Profile Market_Structure_Shift"; }

    @Override
    public String getCategory() { return "market_profile"; }

    @Override
    public String getDescription() { return "Market Profile Market_Structure_Shift - market_profile indicator with period 10"; }
    ...
    public IndicatorResult calculate(List<Candle> candles, String symbol, String timeframe) {
        ...
        double value = IndicatorMath.sma(closes, Math.min(10, closes.size()));
        double high = IndicatorMath.max(closes, Math.min(10, closes.size()));
        double low = IndicatorMath.min(closes, Math.min(10, closes.size()));
        ...
        return new IndicatorResult(getName(), symbol, timeframe,
            ...
            "Market Profile Market_Structure_Shift: value=" + String.format("%.5f", value) + " signal=" + signal);
    }
}
