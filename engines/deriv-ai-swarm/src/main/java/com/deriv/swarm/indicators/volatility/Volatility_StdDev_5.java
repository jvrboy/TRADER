...
public class Volatility_StdDev_5 implements TechnicalIndicator {

    @Override
    public String getName() { return "Volatility StdDev_5"; }

    @Override
    public String getCategory() { return "volatility"; }

    @Override
    public String getDescription() { return "Volatility StdDev_5 - volatility indicator with period 5"; }
    ...
    public IndicatorResult calculate(List<Candle> candles, String symbol, String timeframe) {
        ...
        double value = IndicatorMath.stddev(closes, Math.min(5, closes.size()));
        double price = candles.get(candles.size()-1).getClose();
        double mean = IndicatorMath.sma(closes, Math.min(5, closes.size()));
        ...
        return new IndicatorResult(getName(), symbol, timeframe,
            ...
            "Volatility StdDev_5: value=" + String.format("%.5f", value) + " signal=" + signal);
    }
}
