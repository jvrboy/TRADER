...
/// <summary>
/// Signal generation service using the AI swarm and indicator suite
/// </summary>
public class SignalService : ISignalService
{
    ...
    public async Task<Signal> GenerateSignalAsync(string symbol, string timeframe)
    {
        ...
        return new Signal
        {
            ...
            Strategy = $"Swarm({swarmResult.AgentsUsed} agents) + {driftSignals.Keys.First(k => driftSignals[k] != 0)}",
    ...
    private List<Signal> GenerateDemoSignals()
    {
        var symbols = new[] { ("EURUSD", "Forex"), ("BTCUSD", "Crypto"), ("XAUUSD", "Forex"), ("US500", "Indices"), ("1HZ50V", "Synthetics") };
        var timeframes = new[] { "15", "60", "240", "1440" };
        var signals = new List<Signal>();
        var rng = new Random();

        foreach (var (sym, cat) in symbols)
        {
            var dir = rng.Next(2) == 0 ? SignalDirection.Buy : SignalDirection.Sell;
            var price = 1.0m;
            var score = 60 + rng.Next(35);
            signals.Add(new Signal
            {
                Symbol = sym, Direction = dir,
                Timeframe = timeframes[rng.Next(timeframes.Length)],
                EntryMin = price * 0.999m, EntryMax = price * 1.001m,
                StopLoss = dir == SignalDirection.Buy ? price * 0.995m : price * 1.005m,
                TakeProfit1 = dir == SignalDirection.Buy ? price * 1.005m : price * 0.995m,
                TakeProfit2 = dir == SignalDirection.Buy ? price * 1.010m : price * 0.990m,
                TakeProfit3 = dir == SignalDirection.Buy ? price * 1.015m : price * 0.985m,
                Confidence = score >= 80 ? ConfidenceLevel.VeryHigh : score >= 65 ? ConfidenceLevel.High : ConfidenceLevel.Medium,
                ConfidenceScore = score,
                Status = SignalStatus.Live,
                Category = cat,
                Strategy = "AI Swarm v2.0",
                GeneratedAt = DateTime.UtcNow.AddMinutes(-rng.Next(120))
            });
        }
        return signals;
    }
...
/// <summary>
/// Bot trading service with position management
/// </summary>
...
/// <summary>
/// Deriv WebSocket API service — C# port of deriv-api-helpers.js
/// </summary>
public class DerivApiService : IDerivApiService
{
    ...
    {
        _apiKey = apiKey;
        _isConnected = !string.IsNullOrEmpty(apiKey);
        await Task.CompletedTask;
    }
    ...
    {
        await Task.CompletedTask;
        return symbols.Select(s => new Quote { Symbol = s, Price = 1000m + (decimal)(new Random().NextDouble() * 100) }).ToList();
    }
    ...
    {
        await Task.CompletedTask;
        return new BotTrade { Symbol = symbol, Type = TradeType.Buy, LotSize = amount, Status = TradeStatus.Open, BotName = "Deriv Bot" };
    }
