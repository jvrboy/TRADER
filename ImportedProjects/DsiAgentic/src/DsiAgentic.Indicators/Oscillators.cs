...
/// <summary>
/// 20+ oscillators: RSI, Stochastic, StochRSI, CCI, Williams%R, MFI, CMO, ROC,
/// TSI, Ultimate, Awesome, TRIX, Fisher, Vortex, DPO, KST, ChaikinOsc, PPO,
/// PVO, WilliamsAD, ElderRay, RelativeVigor.
/// </summary>
public static class Oscillators
{
    public static double[] Rsi(double[] c, int p = 14)
    {
        var n = c.Length; var g = new double[n]; var l = new double[n];
    ...
    public static double[] Vortex(double[] h, double[] l, double[] c, int p = 14)
    {
        ...
        var trS = new double[n]; var vpS = new double[n]; var vmS = new double[n];
        for (int i = 1; i < n; i++) { vip[i] = Math.Abs(h[i] - l[i - 1]); vim[i] = Math.Abs(l[i] - h[i - 1]); }
        var res = new double[n];
        for (int i = p; i < n; i++)
        {
            double sVp = 0, sVm = 0, sTr = 0;
            for (int j = i - p + 1; j <= i; j++) { sVp += vip[j]; sVm += vim[j]; sTr += tr[j]; }
